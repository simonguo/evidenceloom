...
def _make_a_share_sentiment_state():
    state = _make_sentiment_state()
    state["company_of_interest"] = "688503.SS"
    return state
...
class TestSentimentAnalystAgent:
    ...
    def test_a_share_prompt_uses_chinese_sources(self, monkeypatch):
        captured = {}
        monkeypatch.setattr(
            sentiment_module,
            "fetch_china_sentiment_sources",
            lambda *args, **kwargs: "东方财富资讯：聚和材料主力资金净流入",
        )
        monkeypatch.setattr(
            sentiment_module,
        ...
        create_sentiment_analyst(_structured_sentiment_llm(captured))(
            _make_a_share_sentiment_state()
        )

        prompt_text = str(captured["prompt"])
        assert "Chinese A-share market sentiment analyst" in prompt_text
        assert "东方财富资讯：聚和材料主力资金净流入" in prompt_text
        assert "skipped for A-share" in prompt_text
